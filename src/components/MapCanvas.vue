<template>
    <div id="mapArea">
        ...
        <unity id="mapScreen" src="../../UnityDev/Build/UnityDev.json" unityLoader="../../UnityDev/Build/UnityLoader.js" ref="unityInstance">
        </unity>
...
<script>
    ...
    export default {
        ...
        watch: {
            webglReady: function (val) {
                if (val == true && !isUpdated) {
                    ...
                    this.$refs.unityInstance.SendMessage('JSConnector', 'UpdateStarBuffer', this.stars);
                    isUpdated = true;
                }
            }
        },
        ...
        mounted() {
            ...
            this.$axios.get('stars/0').then(
                response => this.$unityConnector.SelectedStar = response.data);
        ...
        methods: {
            ...
            apiParamFromPosition() {
                var param = "x=" + this.cameraPos.x + "&y=" + this.cameraPos.y + "&z=" + this.cameraPos.z;
                return param;
            },
            getStarsInRange(range) {
                this.$axios.get('stars/range?' + this.apiParamFromPosition() + '&range=' + range).then((response) => {
